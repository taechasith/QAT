...
export function AvatarPicker({ current, currentUrl, onSaved }: Props) {
  ...
  return (
    <div className="flex flex-col gap-5">
      <p className="text-sm font-medium text-foreground/85">Choose avatar</p>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-2 xl:grid-cols-4">
        {CAT_TYPES.map((type) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-xs font-medium text-foreground/85">{cfg.label}</span>
              <span className="text-[10px] text-muted-foreground">{cfg.sub}</span>
            </button>
          );
        })}
        ...
        >
          ...
          <span className="text-[10px] text-muted-foreground">JPG, PNG · max 2 MB</span>
        </button>
      </div>
      ...
      {msg && (
        <p className={`text-sm ${msg === "Saved!" ? "text-primary" : "text-red-300"}`}>{msg}</p>
      )}

      <button
        onClick={save}
        disabled={saving}
        className="self-start inline-flex h-10 items-center justify-center rounded-full bg-primary px-5 text-sm font-semibold text-slate-950 transition hover:bg-primary/90 disabled:opacity-50"
      >
